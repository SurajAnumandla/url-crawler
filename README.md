...
The PDFs are the design documents. They are generated by `scripts/render_pdf.py` from their sources in [`docs/`](docs/), which are kept so the documents can be diffed and so `scripts/ledger.py --check docs/part2-scale-design.md` can verify every number in Part 2 against the ledger.

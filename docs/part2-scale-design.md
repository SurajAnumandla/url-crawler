...
Rounded estimates, at 10 billion URLs a month across many sites: about 260 servers, about $85,000 a month at the start rising to about $120,000 by the second year as storage accumulates, roughly $8 per million URLs. For the three-site example the same machinery runs on two servers and a few thousand dollars a month, and fetches about 78 million pages a month per site until the sites agree to more. Every figure is an estimate to within about ±50% until the proof of concept measures page size, parse cost and how often pages change. The exact arithmetic is in Appendix B and is generated by a script so it cannot drift from its inputs.
...
|---|---|---|
| Fetches per second, sustained | ~3,900 | 10 billion over 30 days |
| CPU per page | 0.5 s measured on a laptop; 0.75 s assumed for a server core | measured on the two large fixture pages, five runs each |
| Cores needed | ~2,900 | fetch rate × CPU per page |
| Servers deployed | ~260 of 16 cores, run at 70% so there is headroom | |
| Raw HTML stored per month | ~400 TB compressed | 200 KB a page assumed, compressing 5:1 as measured |
| Metadata and body stored per month | ~25 TB compressed | ~10 KB a record measured, compressing 4:1 assumed |
| Frontier table | ~1.2 TB | 10 billion rows of ~120 bytes |
...
| Line | Per month | Notes |
|---|---|---|
| Worker fleet, ~260 Spot servers | ~$60,000 | the dominant line; scales with pages parsed |
| Databases, caches, ingest, API, monitoring | ~$10,000 | fixed; ClickHouse with two replicas, Aurora with its I/O |
| Egress, queue, object writes | ~$2,000 | after the two design choices below |
| Raw HTML storage | ~$9,000 in month 1, ~$28,000 by month 24 | a stock that accumulates; tiered to cold storage |
| Metadata storage | ~$4,000 in month 1, ~$24,000 by month 24 | hot tier replicated, cold tier on S3 |
| **Total** | **~$85,000 in month 1; ~$120,000 by month 24** | about $8 per million URLs at the start |

**Few hosts:** the same components on two worker servers, a fixed floor of about $2,000 a month, roughly $27 per million URLs at 78 million a month. Cost is not the constraint in this regime; the sites' tolerance is.

**The levers, which matter more than the totals:**

| Decision | Effect | Why |
|---|---|---|
| Skip the parse when the raw page is unchanged | up to −60% of the fleet | the parse is the cost; the saving is bounded by how often pages actually change |
| Pack raw pages into large objects | −99.9% of object-write charges | 1,000 pages per write instead of one |
| Public IP egress instead of a NAT gateway | −95% of egress cost | NAT charges per byte; ~400 TB a month of compressed pages |
...
| No key-value replica for serving | avoids ~$125,000 a month | the sorted column store already answers single-URL lookups |
...
7. Frontier refill latency at a million hosts on a 1.2 TB table, and the re-release path after a Redis failover.
8. A fair column-store versus row-store comparison on real hardware before anyone quotes a speed multiple.
...
Generated by `scripts/ledger.py --compact`; the arithmetic for each derived row is in that script, and `--check` verifies that every value below appears in this document. Labels: **measured** (where and how), **assumed** (why this value), **derived** (arithmetic from other rows).
...
| N-09 | Compute per month / per M | $59,443 / $5.94 | derived |
...
| N-12 | Raw S3 bill at 1 / 12 / 24 months | $9,200 / $22,764 / $27,516 per month | derived |
| N-13 | ClickHouse storage at 1 / 12 / 24 months (hot tier × 2 replicas) | $4,000 / $17,175 / $24,075 per month | derived |
...
| N-15b | Public IPv4 instead of NAT | $929/mo | derived |
| N-16 | SQS | $1,200/mo batched; $12,000 unbatched | derived |
| N-17 | Dedup saving at 60% unchanged (upper bound) | $35,666/mo compute + $30 PUT | derived |
...
| N-21 | DynamoDB full replica writes | $125,000/mo | derived |
| N-21b | DynamoDB replica storage | $25,000/mo, accumulating | derived |
| N-24 | Per million URLs (NAT) / (public IPv4) | $8.99 / $7.28 | derived |
| N-24e | Fixed infrastructure, many-domain (incl. Aurora I/O, ingest, cross-AZ) | $9,523/mo | assumed |
| N-29 | Frontier table (10e9 rows) | 1.2 TB; $120/mo | derived |
| N-24f | Total month 1 / per M all lines month 1 | $84,465 / $8.45 | derived |
| N-24g | Variable flat lines (compute + PUT + IPv4 + SQS) | $61,622 | derived |
| N-24c | Flat lines / total at 12 / 24 months | $71,265 / $111,204 / $122,856 | derived |
| N-24d | IPv4 per M / KV summary tier | $0.09 / $12,500 | derived |
...
| N-27 | Lambda comparison | $64,500/mo | derived |
...
| F-20 | Fixed monthly floor | $2,038 | assumed |
| F-21 | Variable per month at 10 req/s/host | $87.09 | derived |
| F-22 | Per million URLs at 10 req/s/host | $27.33 | derived |
